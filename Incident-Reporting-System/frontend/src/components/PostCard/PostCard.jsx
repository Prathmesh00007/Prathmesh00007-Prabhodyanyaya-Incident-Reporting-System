...
const PostCard = ({ post, onDelete, onVoteUpdate }) => {
  ...
  const handleVote = async (voteType) => {
    setVoteLoading(true);
    try {
      const response = await fetch(`https://prathmesh00007-prabhodyanyaya-incident.onrender.com/api/community/posts/${post._id}/vote`, {
        method: 'POST',
        headers: {
  ...
  const handleAddComment = async (e) => {
    ...
    setLoading(true);
    try {
      const response = await fetch(`https://prathmesh00007-prabhodyanyaya-incident.onrender.com/api/community/posts/${post._id}/comments`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${localStorage.getItem('token')}`
  ...
  const handleDelete = async () => {
    ...
    try {
      const response = await fetch(`https://prathmesh00007-prabhodyanyaya-incident.onrender.com/api/community/posts/${post._id}`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('token')}`
        }
  ...
  const handleReport = async () => {
    ...
    try {
      const response = await fetch(`https://prathmesh00007-prabhodyanyaya-incident.onrender.com/api/community/posts/${post._id}/report`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${localStorage.getItem('token')}`
