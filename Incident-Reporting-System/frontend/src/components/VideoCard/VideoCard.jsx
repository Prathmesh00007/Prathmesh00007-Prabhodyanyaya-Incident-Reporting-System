...
const VideoCard = ({ video, onDelete, onVoteUpdate }) => {
  ...
  const handleVote = async (voteType) => {
    ...
    try {
      setVoting(true);
      const response = await fetch(`https://prathmesh00007-prabhodyanyaya-incident.onrender.com/api/videos/${video._id}/vote`, {
        method: 'POST',
        headers: {
  ...
  const handleDelete = async () => {
    ...
    try {
      const response = await fetch(`https://prathmesh00007-prabhodyanyaya-incident.onrender.com/api/videos/${video._id}`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('token')}`
        }
    ...
    <div className="bg-white rounded-lg shadow-sm hover:shadow-md transition-shadow overflow-hidden">
      ...
      <Link to={`/videos/${video._id}`} className="block relative">
        <div className="aspect-video bg-gray-200 relative overflow-hidden">
          {video.thumbnailUrl ? (
            <img 
              src={`https://prathmesh00007-prabhodyanyaya-incident.onrender.com/${video.thumbnailUrl}`}
              alt={video.title}
